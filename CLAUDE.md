...
The Python bridge `Scripts/agent-isle-hook.py` is the Claude Code end of this: it is registered as `PreToolUse`/`PostToolUse`/`Notification`/`Stop`/`UserPromptSubmit` hooks. `PreToolUse` blocks (300s timeout) awaiting the notch decision and echoes it back in Claude's `hookSpecificOutput` format. Its `should_ask()` mirrors when Claude Code would actually prompt so the island intercepts the same requests Claude would — keep this in sync with Claude Code's permission behavior. Because `PreToolUse` runs *before* Claude Code evaluates its permission rules, and a hook `allow` short-circuits that evaluation, `should_ask()` has to re-derive the verdict itself: `rule_verdict()` reads the user's own `permissions.allow`/`ask`/`deny` rules from the merged settings files (managed policy, `CLAUDE_CONFIG_DIR` user settings, and every `.claude/settings{,.local}.json` from the project root down to the session cwd, the root being the outermost enclosing repo — or, with no repo, the outermost `.claude`/language-manifest directory — searched strictly below `$HOME` and below any top-level directory, so a monorepo package never shadows the repo root's settings while neither a dotfiles repo at `$HOME` nor any unrelated ancestor can contribute rules), so anything Claude Code recorded as "don't ask again" no longer gets re-prompted in the notch. Only when no rule applies does it fall back to skipping read-only tools and respecting `acceptEdits`/`plan`/`bypassPermissions`. Rule matching is deliberately conservative — an unrecognized specifier shape never counts as pre-approved, and shell operators are judged per part: what the rule itself spells out is approved (so a rule that chains matches its own chain), but the tail a prefix rule leaves unspecified must be operator-free, and a multiline command never matches a single-line rule. When in doubt the island asks rather than silently allowing. A `deny` match also skips the prompt, leaving Claude Code to do the blocking. `AGENT_ISLE_APPROVALS=0` disables gating; `AGENT_ISLE_IGNORE_RULES=1` ignores the settings rules and uses the mode heuristics alone; `AGENT_ISLE_DEBUG=1` logs payloads to `~/.agent-isle/hook-debug.jsonl` (mode `0600`). The hook must never break the user's session, so all failures are swallowed and it exits 0.
